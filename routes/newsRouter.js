...
// ✅ FIXED: Main webhook endpoint with correct Telex format
router.all("/a2a/:channelId/message", async (req, res) => {
  try {
    ...
    if (text && text.toLowerCase().includes('news')) {
      ...
      // ✅ FIXED: Use "messages" array with "kind: text"
      return res.json({
        jsonrpc: "2.0",
        result: {
          messages: [
  ...
  try {
    ...
    res.json({
      ...
      result: { 
        messages: [{ 
          kind: "text", 
          text: "Ask me for 'news' to get the latest tech headlines! 📰" 
        }] 
      },
      id: a2aRequest.id
    });
