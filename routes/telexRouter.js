...
// ✅ Main webhook endpoint for Telex messages
...
// ✅ Mastra A2A endpoint for agent-to-agent messages
router.post("/a2a/agent/:agentId", async (req, res) => {
  try {
    ...
    if (text && text.toLowerCase().includes("news")) {
      ...
      const response = newsResult.success
        ? newsResult.headlines.map(h => `📰 ${h.title}\n🔗 ${h.url}`).join("\n\n")
        : "No news available.";

      return res.json({
        jsonrpc: "2.0",
        result: {
          messages: [{ kind: "text", text: response }],
        },
        id: a2aRequest.id,
      });
    }
...
router.get("/register", async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      instructions: "Use the webhook endpoint for messages or A2A endpoint for Mastra node format",
    });
  } catch (error) {
